...
function update() {
  ...
  tb2.innerHTML =
    Math.round((totalCPUsAllocated * 100) / totalCPUsAvailable) + "%";
  ...
  status = checkStatus();
  t.children[1].children[7].lastChild.innerHTML =
    status.length > 0 ? "Warning<br>" + status : "Good";
  t.children[1].children[7].lastChild.style.color =
    status.length > 0 ? "red" : "#77dd77";
...
function addNodes() {
  ...
  inf.innerHTML =
    "This Cluster is using " + type + "typed Nodes. Which means:<br>";
  inf.innerHTML += "Each Node will have<br>";
  inf.innerHTML += "---->CPUs : 16<br>";
  inf.innerHTML +=
    type == "Large" ? "---->Memory : 560 GB<br>" : "---->Memory : 384 GB<br>";
  inf.innerHTML +=
    type == "Large" ? "---->Disk : 18 TB<br>" : "---->Disk : 15 TB<br>";
...
function addN() {
  ...
  t.children[1].children[4].lastChild.innerHTML =
    Math.round((totalCPUsAllocated * 100) / totalCPUsAvailable) + "%";

  totalMemory = type == "Large" ? totalMemory + n * 560 : totalMemory + n * 384;
  t.children[1].children[5].lastChild.innerHTML =
    Math.round((totalMemoryUsed * 100) / totalMemory) + "%";

  totalDisk = type == "Large" ? totalDisk + n * 18000 : totalDisk + n * 15000;
  t.children[1].children[6].lastChild.innerHTML =
    Math.round((usedDisk * 100) / totalDisk) + "%";
  status = checkStatus();
  t.children[1].children[7].lastChild.innerHTML =
    status.length > 0 ? "Warning<br>" + status : "Good";
  t.children[1].children[7].lastChild.style.color =
...
function checkStatus() {
  ...
  if (
    localStorage.diskMaxDefault &&
    Math.round((usedDisk * 100) / totalDisk) > localStorage.diskMaxDefault
  ) {
...
function checkStatus2(cpuU, memoryU, diskU) {
  ...
  if (
    localStorage.memoryMaxDefault &&
    memoryU > localStorage.memoryMaxDefault
  ) {
    status += "Memory exceeded " + localStorage.memoryMaxDefault + "%<br>";
  } else {
    if (memoryU > 100) {
...
function displayWarning(totCpus, totMemory, totDisk) {
  ...
  Object.values(data).forEach((cluster) => {
    for (let i = 0; i < cluster.length; i++) {
      //console.log(vms)
      cpuU = Math.round(
        ((parseInt(cluster[i].totalCPUsAllocated) + totCpus) * 100) /
          parseInt(cluster[i].totalCPUsAvailable)
      );
      memoryU = Math.round(
        ((parseInt(cluster[i].totalMemoryUsed) + totMemory) * 100) /
          parseInt(cluster[i].totalMemory)
      );
      diskU = Math.round(
        ((parseInt(vms[i][0]) + totDisk) * 100) / parseInt(vms[i][1])
      );
      let tr = document.createElement("tr");
...
function exportPDF() {
  ...
  var opt = {
    ...
    filename: 		"clustersinfo.pdf",
    image: 			  { type: "png" },
    html2canvas: 	{ width: 1200, y: 400, scrollY: 400 },
